...
struct Instruction {
    ...
    std::string to_string() const {
        ...
        case LdScratch :
            ...
            break;
        case StScratch :
            result = "StScratch #" + std::to_string(decode_wide_index()) + reg_to_string();
            break;
        case AddFeature :
            result = "AddFeature #" + std::to_string(decode_wide_index()) + reg_to_string();
            break;
        case SubFeature :
            result = "SubFeature #" + std::to_string(decode_wide_index()) + reg_to_string();
            break;
        case Finalize :
            result = "Finalize #" + std::to_string(decode_bucket());
            break;
    ...
    static constexpr Instruction reset_reg(Reg regDst) {
        return {uint32_t(ResetReg + (regDst << OpcodeBits))};
    }
   ...
   private:
    std::string reg_to_string() const { return std::string(", ") + "ABCD"[decode_reg()]; }
};
}
